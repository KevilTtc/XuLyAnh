...
import com.example.edit.view.VerticalSeekBar;
...
public class ImageFilterActivity extends Activity implements View.OnClickListener {
    ...
    private void updatePicture(float degree)
    {
        ...
        int[] pix = new int[srcWidth * srcHeight]; // pixel thành phần nhận màu của bitmap
        /*
         * @param pixels   The array to receive the bitmap's colors
         * @param offset   The first index to write into pixels[]
         * @param stride   sải bước Số lượng mục nhập tính bằng pixel []
         * @param x        @param x Tọa độ x của pixel đầu tiên để đọc từ đó
         *
         * @param y        @param y Tọa độ y của pixel đầu tiên để đọc từ đó
         * @param width    The number of pixels to read from each row
         * @param height   The number of rows to read
         */
        newBitmap.getPixels(pix, 0, srcWidth, 0, 0, srcWidth, srcHeight);

        // degree Bộ lọc hình ảnh thay đổi mức độ( 0 < factor < 1)
        switch (filterType)
        {
